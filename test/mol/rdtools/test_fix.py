...
def test_saturate_mol(smi, add_hs, exp_smi, exp_mult):
    ...
    assert mol.GetSpinMultiplicity() == exp_mult
    assert mol.ToSmiles() == exp_smi
...
def test_fix_sanitize_ok(smi, add_hs, exp_smi):
    ...
    assert fixed_mol.ToSmiles() == exp_smi
...
def test_fix_sanitize_bad_non_resonance(smi, add_hs, exp_smi):
    mol = RDKitMol.FromSmiles(smi, addHs=add_hs, sanitize=False)
...
def test_fix_mol_complex():
    mol = RDKitMol.FromSmiles("[CH2][CH2].[C]=O")

    fixed_mols = fix_mol(
        mol, remedy_manager.all_remedies
    ).GetMolFrags(asMols=True)

    assert set(fixed_mol.ToSmiles() for fixed_mol in fixed_mols) == set(
        ["C=C", "[C-]#[O+]"]
    )
...
def test_renumber_after_fix():
    ...
    assert mol.GetAtomMapNumbers() != fixed_mol.GetAtomMapNumbers()
