...
from rdmc.resonance.resonance_rmg import (generate_resonance_structures,
                                          generate_allyl_delocalization_resonance_structures)


def test_generate_allyl_delocalization_resonance_structures():
    """
    Test the function for generating allyl delocalization resonance structures.
    """
    smi = '[CH2]C=CC=C'
    assert len(
        generate_allyl_delocalization_resonance_structures(
...
class TestResonance:
    ...
    def test_allyl_shift(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("C=C[CH]C=CC"))
        assert len(mol_list) == 3

    def test_trirad_allyl_shift(self):
        """Test allyl shift for a tri-rad carbon"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[C]N=N"))
        assert len(mol_list) == 3
        assert any([any([atom.GetFormalCharge() != 0 for atom in mol.GetAtoms()]) for mol in mol_list])  # expecting [C]=[N+.][NH-]

    def test_oxime(self):
        """Test resonance structure generation for CC=N[O] radical

        Simple case for lone pair <=> radical resonance"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("CC=N[O]"))
        assert len(mol_list) == 3
        assert any([any([atom.GetFormalCharge() != 0 for atom in mol.GetAtoms()]) for mol in mol_list])

    # def test_ring_allyl_shift(self):
    #     """Test allyl shift for a cyclic species with heteroatoms"""
    #     mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[CH]1C=NC=N1"))
    #     assert len(mol_list) == 5

    def test_carbene_allyl_shift(self):
        """Test allyl shift for a carbene species"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[C]=C=O"))
        assert len(mol_list) == 2

    def test_ch2chcho(self):
        """Test resonance structure generation for C=C[CH][O] bi-radical"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("C=C[CH][O]"))
        assert len(mol_list) == 3

    def test_ch2no(self):
        """Test combined resonance transitions of allyl-shift and lonePair-radical"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[CH2]N=O"))
        assert len(mol_list) == 3

    def test_ch3s2o2(self):
        """Test combined resonance transitions of one_pair_radical_multiple_bond"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("CSS(=O)[O]"))
        assert len(mol_list) == 3

    def test_n2so2(self):
        """Test the resonance transitions of a species with several hereroatoms and several multiple bonds"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[N-]=[N+]=S(=O)=O"))
        assert len(mol_list) == 2

    def test_nsh(self):
        """Test that a resonance structure with a minimal octet deviation but higher charge span is filtered out"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("N#S"))
    ...
    def test_nco(self):
        ...
        """
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[N]=C=O"))
        assert len(mol_list) == 2
        assert all([all([atom.GetFormalCharge() == 0 for atom in mol.GetAtoms()]) for mol in mol_list])  # none of the
    ...
    def test_no2(self):
        ...
        """
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[O]N=O"))
        assert len(mol_list) == 2
        assert any([any([atom.GetFormalCharge() != 0 for atom in mol.GetAtoms()]) for mol in mol_list])  # one of the
        # structures should be charged
    ...
    def test_n2o(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("N#[N+][O-]"))
        assert len(mol_list) == 2
        assert all([any([atom.GetFormalCharge() != 0 for atom in mol.GetAtoms()]) for mol in mol_list])  # both structures
    ...
    def test_azide(self):
        ...
        """
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("CCN=[N+]=[N-]"))
        assert len(mol_list) == 2
        assert all([any([atom.GetFormalCharge() != 0 for atom in mol.GetAtoms()]) for mol in mol_list])
    ...
    def test_ozone(self):
        ...
        """
        mol_list_1 = generate_resonance_structures(RDKitMol.FromSmiles("[O-][O+]=O"))
        assert len(mol_list_1) == 1
        mol_list_2 = generate_resonance_structures(RDKitMol.FromSmiles("O=S=O"))
        assert len(mol_list_2) == 1
        mol_list_3 = generate_resonance_structures(RDKitMol.FromSmiles("S=S=S"))
    ...
    def test_hco_vs_hcs(self):
        ...
        """
        mol_list_1 = generate_resonance_structures(RDKitMol.FromSmiles("[CH]=O"))
        assert len(mol_list_1) == 1
        mol_list_2 = generate_resonance_structures(RDKitMol.FromSmiles("[CH]=S"))
    ...
    def test_no(self):
        ...
        """
        mol = RDKitMol.FromSmiles('[N][O]')
        mol.GetAtomWithIdx(0).SetNumRadicalElectrons(0)
        mol_list = generate_resonance_structures(mol)

        assert len(mol_list) == 2
        assert get_lone_pair(mol_list[0].GetAtoms()[0]) + get_lone_pair(mol_list[0].GetAtoms()[1]) == 3
        assert get_lone_pair(mol_list[1].GetAtoms()[0]) + get_lone_pair(mol_list[1].GetAtoms()[1]) == 4
    ...
    def test_n5dc_radical(self):
        ...
        """
        mol = RDKitMol.FromSmiles("N=[N+]([O-])[O]")
    ...
    def test_styryl1(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("c1ccccc1[C]=C"))
        assert len(mol_list) == 3
    ...
    def test_styryl2(self):
        ...
        In this case, the radical can be delocalized into the aromatic ring"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("C=C=C1C=C[CH]C=C1"))
        assert len(mol_list) == 3
    ...
    #     """
    #     mol_list = generate_resonance_structures(RDKitMol.FromSmiles("c12[c]cccc1cccc2"))
    ...
    #     mol_list = generate_resonance_structures(RDKitMol.FromSmiles("CC1=CC=CC2=CC=CC=C12"))
    #     assert len(mol_list) == 4
    ...
    def test_methyl_phenanthrene(self):
        ...
        Example of stable polycyclic aromatic species"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("CC1=CC=CC2C3=CC=CC=C3C=CC=21"))
        assert len(mol_list) == 3
    ...
    def test_methyl_phenanthrene_radical(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[CH2]C1=CC=CC2C3=CC=CC=C3C=CC=21"))
    ...
    def test_aromatic_with_lone_pair_resonance(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("c1ccccc1CC=N[O]"))
        assert len(mol_list) == 4

    def test_aromatic_with_n_resonance(self):
        """Test resonance structure generation for aromatic species with lone pair resonance"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("c1ccccc1CCN=[N+]=[N-]"))
        assert len(mol_list) == 2
...
#         assert len(mol_list) == 1
    ...
    def test_no_clar_structures(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("C1=CC=CC2C3=CC=CC=C3C=CC=21"), clar_structures=False)
        assert len(mol_list) == 2
...
#         mol_list = generate_resonance_structures(RDKitMol.FromSmiles("[CH](c1ccccc1)c1ccc(C)cc1"))
#         assert len(mol_list) == 6
    ...
    def test_c8h8(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("C=C1C=CC=CC1=C"))
        assert len(mol_list) == 1
    ...
    def test_c8h7_j(self):
        ...
        Example of molecule that RDKit considers aromatic, but RMG does not"""
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("C=C1C=CC=CC1=[CH]"))
        assert len(mol_list) == 1
...
#         mol1 = RDKitMol.FromSmiles("C1=CC=C=C=C1")
#         mol2 = RDKitMol.FromSmiles("C1C#CC=CC=1")
...
#         mol1 = RDKitMol.FromSmiles("C12=CC=C=C=C1C=CC=C2")
#         mol2 = RDKitMol.FromSmiles("C12C#CC=CC=1C=CC=C2")
...
#         mol = RDKitMol.FromSmiles("C12C#CC=CC=1C=CC3=C2C=CC=C3")
...
#         mol = RDKitMol.FromSmiles("c12c3cccc1c3ccc2")
...
#         Update: Highly-strained fused rings are no longer considered aromatic."""
#         mol = RDKitMol.FromSmiles("c1c2cccc1C(=C)C=[C]2")
#         arom = Molecule().from_adjacency_list(
#             """
# multiplicity 2
...
#         This applies in cases where RDKit misidentifies one ring as aromatic, but there are other
#         rings in the molecule that are actually aromatic."""
#         mol = RDKitMol.FromSmiles("C=C(C1=CC2=C(C=C1C=C3)C4=CC5=CC=CC=C5C=C4C=C2)C3=C")
#         arom = Molecule().from_adjacency_list(
#             """
...
#         mol = RDKitMol.FromSmiles("C=C[CH2]")
#         mol.assign_atom_ids()
...
#         mol = RDKitMol.FromSmiles("[CH2]C=C1C=CC(=C)C=C1")
#         out = generate_resonance_structures(mol)
...
#         mol = RDKitMol.FromSmiles("C=C1C=CC=C2C=C[CH]C=C12")
#         out = generate_resonance_structures(mol)
...
#         mol = RDKitMol.FromSmiles("[CH2]C=C1C=CC(=C)C2=C1C=CC=C2")
#         out = generate_resonance_structures(mol)
...
#         mol = RDKitMol.FromSmiles("C=C1C=CC=CC1=C")
#         newmol = generate_clar_structures(mol)
...
#         assert len(mol_list) == 1
    ...
    def test_surface_radical(self):
        ...
        mol_list = generate_resonance_structures(RDKitMol.FromSmiles("*=C[CH]C"))
        assert len(mol_list) == 1
