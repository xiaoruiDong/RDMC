...
def match_Hs(r_info:list,
    ...
    if sum(Hs_number) !=0:
        ...
        warnings.warn("Some hydrogen atoms might migrate from/to [H] for [H][H]")
    elif r_sep_Hs_info or p_sep_Hs_info:
        Sep_Hs = True # If not balanced H for heavy atoms, H migrated from/to heavy atoms
        warnings.warn("Some hydrogen atoms might migrate from/to [H] for [H][H], however, these atoms might be ambiguous")
